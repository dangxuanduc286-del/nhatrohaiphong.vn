...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!page) {
    return createSeoMetadata({
      ...
      description: "Landing page không tồn tại hoặc chưa được xuất bản.",
      path,
      noIndex: true,
    });
  }
  ...
  return createSeoMetadata({
    ...
    keywords: seo?.keywords?.length
      ? seo.keywords
      : [page.title, page.city?.name, page.district?.name, page.poi?.name].filter(
...
export default async function LandingPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const breadcrumbs = [
    { name: "Trang chủ", url: "/" },
    {
      name: page.city?.name ?? "Hải Phòng",
      url: page.city?.slug === "hai-phong" ? "/phong-tro-hai-phong" : page.path,
    },
  ...
  return (
    <main className="min-h-screen bg-[#F9FAFB] text-[#111827]">
      <AnalyticsView
        ...
        payload={{ location: "seo_landing_page", path: page.path, title: page.title }}
      />
          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_320px] lg:items-start">
            <div>
              <p className="text-sm font-normal uppercase tracking-wide text-[#2563EB]">
                Landing page Hải Phòng
              </p>
              <h1 className="mt-4 text-3xl font-extrabold tracking-tight text-[#111827] sm:text-4xl lg:text-5xl">
                {page.title}
              </h1>
              <p className="mt-5 max-w-3xl text-base font-normal leading-7 text-[#6B7280]">
                {page.content ?? buildLandingDescription(page)}
              </p>
              <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                <Link
                  href="#danh-sach-phong"
                  data-analytics-event="cta_find_room_click"
                  data-analytics-location="seo_landing_hero"
                  data-analytics-label="Xem phòng phù hợp"
                ...
                <Link
                  ...
                  data-analytics-location="seo_landing_hero"
                  data-analytics-label="Nâng cấp Chủ trọ"
                  className="inline-flex min-h-12 items-center justify-center rounded-2xl border border-[#EA580C] bg-white px-5 text-sm font-bold text-[#EA580C] shadow-sm"
                >
                  Nâng cấp Chủ trọ
                </Link>
              </div>
              <div className="mt-6 flex flex-wrap gap-3 text-sm font-normal text-[#6B7280]">
                {page.city ? (
                  <span className="rounded-full bg-blue-50 px-4 py-2 text-[#2563EB]">
                    {page.city.name}
                  </span>
                ) : null}
                {page.district ? (
                  <span className="rounded-full bg-slate-100 px-4 py-2 text-[#6B7280]">
                    {page.district.name}
                  </span>
                ) : null}
                {page.poi ? (
                  <span className="rounded-full bg-orange-50 px-4 py-2 text-[#EA580C]">
                    Gần {page.poi.name}
                  </span>
                ) : null}
              </div>
            ...
            <aside className="rounded-2xl border bg-white p-6 shadow-sm transition-all hover:shadow-lg">
              ...
              <dl className="mt-4 grid gap-3 text-sm font-normal">
                <div className="flex justify-between gap-4">
                  <dt className="text-[#6B7280]">Số phòng phù hợp</dt>
                  <dd className="font-semibold text-[#111827]">{page.search.total}</dd>
                </div>
                <div className="flex justify-between gap-4">
                  <dt className="text-[#6B7280]">Trang chuẩn</dt>
                ...
                {page.poi ? (
                  <div className="flex justify-between gap-4">
                    <dt className="text-[#6B7280]">Bán kính POI</dt>
                    <dd className="font-semibold text-[#111827]">5 km</dd>
                  </div>
                ) : null}
              </dl>
        ...
        <div className="flex items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm font-normal text-[#6B7280]">
              Danh sách được lấy từ dữ liệu phòng đang khả dụng, ưu tiên cập nhật mới.
            </p>
          </div>
          <Link
            href={page.path}
            className="hidden min-h-12 items-center rounded-2xl border bg-white px-4 py-2 text-sm font-semibold text-[#2563EB] transition-all hover:shadow-lg sm:inline-flex"
          >
            Xem trang khu vực
          </Link>
        </div>
          ...
          {rooms.length > 0 ? (
            rooms.map((room) => (
              ...
              >
                ...
                <p className="mt-2 line-clamp-2 text-sm font-normal text-[#6B7280]">
                  {room.address}
                </p>
                ...
                <p className="mt-4 text-lg font-semibold text-[#EA580C]">
                  {room.price.toLocaleString("vi-VN")} đ/tháng
                </p>
                <Link
                  href={`/phong/${room.slug}`}
                  data-analytics-event="landing_room_click"
                  data-analytics-location="seo_landing_room_list"
                  data-analytics-label={room.title}
                  className="mt-4 inline-flex min-h-11 items-center justify-center rounded-2xl bg-[#2563EB] px-4 text-sm font-bold text-white"
                >
                  Xem chi tiết
                </Link>
              </article>
            ...
            <div className="rounded-2xl border bg-white p-6 text-sm font-normal text-[#6B7280] md:col-span-2 lg:col-span-3">
              Khu vực này đang được cập nhật phòng trọ mới.
            </div>
          )}
        </div>
      </section>
            ...
            <div className="mt-5 grid gap-3">
              {relatedLinks.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="rounded-2xl border p-6 shadow-sm transition-all hover:shadow-lg"
                >
            ...
            <div className="mt-5 space-y-4">
              {page.faqs.length > 0 ? (
                ...
                <p className="text-sm font-normal leading-6 text-[#6B7280]">
                  Thông tin hỏi đáp đang được cập nhật theo dữ liệu thực tế của khu vực.
                </p>
              )}
            </div>
          </div>
      ...
      <section className="mx-auto max-w-6xl px-4 pb-12 sm:px-6 lg:px-8 lg:pb-20">
        <div className="rounded-3xl bg-[#111827] p-6 text-white shadow-xl sm:p-8 lg:flex lg:items-center lg:justify-between">
          <div>
            <p className="text-sm font-bold uppercase tracking-wide text-orange-200">
              Tăng tốc hành trình
            </p>
            <h2 className="mt-2 text-2xl font-extrabold sm:text-3xl">
              Xem phòng phù hợp hoặc đăng tin cho khu vực này
            </h2>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-200">
              CTA cuối trang giúp người dùng không phải cuộn lại đầu trang để tiếp tục hành động.
            </p>
          </div>
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:mt-0">
            <Link
              href="#danh-sach-phong"
              data-analytics-event="cta_find_room_click"
              data-analytics-location="seo_landing_final_cta"
              data-analytics-label="Xem phòng"
              className="inline-flex min-h-12 items-center justify-center rounded-2xl bg-white px-5 text-sm font-extrabold text-[#2563EB]"
            >
              Xem phòng
            </Link>
            <Link
              href="/login?next=/landlord/rooms/new"
              data-analytics-event="cta_post_room_click"
              data-analytics-location="seo_landing_final_cta"
              data-analytics-label="Đăng phòng"
