...
export default async function AdminPropertiesPage({
  ...
  const normalizedStatus: RoomStatus | undefined =
    status === "AVAILABLE" ||
    status === "OCCUPIED" ||
    status === "RESERVED" ||
    status === "MAINTENANCE" ||
  ...
  const rooms = await db.room.findMany({
    ...
    select: {
      ...
      building: {
        select: {
          property: { select: { name: true, owner: { select: { fullName: true, email: true } } } },
        },
      },
  ...
  const auditLogs = await db.auditLog.findMany({
    where: { entityType: "Room", entityId: { in: rooms.map((room) => room.id) } },
    orderBy: { createdAt: "desc" },
    take: 20,
  });
  ...
  return (
    <section className="space-y-6">
      <div>
        ...
        <h2 className="text-2xl font-bold">Quản lý tài sản/phòng trọ</h2>
        <p className="mt-2 max-w-3xl text-sm text-slate-600">
          Search/filter/status/owner/date, action view/moderate/audit history. Không xóa dữ liệu.
        </p>
      </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {rooms.map((room) => (
                <tr key={room.id} className="align-top">
                  ...
                  <td className="px-4 py-3">
                    <div>{room.building.property.owner.fullName}</div>
                    <div className="text-xs text-slate-500">
                      {room.building.property.owner.email}
                    </div>
                    <div className="text-xs text-slate-500">{room.building.property.name}</div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-slate-600">{formatDate(room.createdAt)}</td>
                  <td className="px-4 py-3">{room.reports.length}</td>
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-2">
                      <Link
                        className="rounded-lg border px-2 py-1 text-xs font-semibold"
                        href={`/admin/rooms?search=${room.roomCode}`}
                      >
                        View
                      </Link>
                      <form action={moderateRoomAction}>
                        ...
                        <AdminActionButton
                          label="Moderate"
                          message={`Ẩn phòng ${room.roomCode}?`}
                        />
                      </form>
                    </div>
                  </td>
      ...
      <div className="rounded-2xl border bg-white p-5 shadow-sm">
        <h3 className="font-semibold">Audit history gần đây</h3>
