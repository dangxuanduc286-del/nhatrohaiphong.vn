...
export default async function AdminLandingPagesPage({
  ...
  const columns: AdminTableColumn<LandingPageRow>[] = [
    ...
    },
    {
      ...
      render: (item) => (
        <div>
          <div>{item.seoSettings[0]?.metaTitle ?? "—"}</div>
          <div className="text-xs text-slate-500">
            {item.seoSettings[0]?.canonicalUrl ?? "No canonical"}
          </div>
        </div>
      ),
    },
    {
      key: "location",
      label: "Location/POI",
      render: (item) => item.city?.name ?? item.district?.name ?? item.poi?.name ?? "Global",
    },
    {
      key: "status",
      label: "Status",
  ...
  return (
    <section className="space-y-6">
      <div>
        ...
        <h2 className="text-2xl font-bold">Quản lý landing pages</h2>
        <p className="mt-2 text-sm text-slate-600">
          CRUD qua API admin; UI danh sách hiển thị SEO Title, Meta, Canonical, Slug và Status.
        </p>
      </div>
      <AdminFilters>
      ...
      <AdminDataTable
        columns={columns}
        items={data.items}
        empty={<AdminEmptyState title="Không có landing page" />}
      />
      <AdminPagination
        page={data.page}
