...
async function getUsers(searchParams: URLSearchParams) {
  ...
  const normalizedStatus: UserStatus | undefined =
    status === "ACTIVE" || status === "INACTIVE" || status === "BANNED" ? status : undefined;
  const where = {
...
export default async function AdminUsersPage({
  ...
  searchParams?: Promise<{ search?: string; status?: string; deleted?: string; page?: string }>;
}) {
  ...
  const columns: AdminTableColumn<UserRow>[] = [
    {
      ...
      render: (user) => (
        <div>
          ...
          <div className="text-xs text-slate-500">{user.phone ?? "Chưa có phone"}</div>
          {user.deletedAt ? (
    ...
    },
    {
      ...
      render: (user) => (
        <div className="flex flex-wrap gap-1">
          {user.roles.map((item) => (
            <span key={item.roleId} className="rounded-full bg-slate-100 px-2 py-1 text-xs">
              {item.role.slug}
            </span>
          ))}
        </div>
    ...
    },
    {
      ...
      render: (user) => (
        <AdminStatusBadge
          value={user.lockedUntil ? "LOCKED" : user.status}
          tone={user.lockedUntil ? "red" : statusTone(user.status)}
        />
      ),
    },
    {
      key: "sessions",
      label: "Sessions",
      render: (user) => user.sessions.length.toLocaleString("vi-VN"),
    },
    {
      key: "lastLogin",
      label: "Đăng nhập cuối",
      render: (user) => <span className="text-slate-600">{formatDate(user.lastLoginAt)}</span>,
    },
    {
      key: "actions",
      label: "Actions",
    ...
    <section className="space-y-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm text-slate-600">
            Khóa/mở khóa, soft delete và restore qua server action có permission user.manage. Không
            hard delete user.
          </p>
        </div>
      </div>
      <div className="grid gap-3 sm:grid-cols-4">
        <div className="rounded-2xl border bg-white p-4 shadow-sm">
          ...
          <strong className="text-2xl">{data.totalUsers.toLocaleString("vi-VN")}</strong>
        </div>
        <div className="rounded-2xl border bg-white p-4 shadow-sm">
        ...
        <div className="rounded-2xl border bg-white p-4 shadow-sm">
          ...
          <strong className="text-2xl text-rose-700">
            {data.bannedUsers.toLocaleString("vi-VN")}
          </strong>
        </div>
        <div className="rounded-2xl border bg-white p-4 shadow-sm">
          <p className="text-xs text-slate-500">Đã xóa mềm</p>
          <strong className="text-2xl text-amber-700">
            {data.deletedUsers.toLocaleString("vi-VN")}
          </strong>
        </div>
      </div>
      <AdminFilters>
      ...
      <AdminDataTable
        columns={columns}
        items={data.items}
        empty={<AdminEmptyState title="Không có user" />}
      />
      <AdminPagination
        page={data.page}
        totalPages={data.totalPages}
        basePath="/admin/users"
        searchParams={{ search: data.search, status: data.status, deleted: data.deleted }}
      />
    </section>
  );
}
