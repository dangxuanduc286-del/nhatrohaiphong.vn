...
export default async function AdminRolesPage({
  ...
  await requireAdminPage("role.manage");
  ...
  const columns: AdminTableColumn<RoleRow>[] = [
    ...
    },
    {
      ...
      render: (item) => (
        <div>
          <div className="font-semibold">{item._count.permissions}</div>
          <div className="mt-1 flex flex-wrap gap-1">
            {item.permissions.slice(0, 6).map((rolePermission) => (
              <span
    ...
    },
    {
      ...
      render: (item) => (
        <div>
          <div className="font-semibold">{item._count.users}</div>
          <div className="mt-1 space-y-1 text-xs text-slate-500">
            {item.users.slice(0, 3).map((userRole) => (
              <div key={userRole.userId}>
                {userRole.user.fullName} · {userRole.user.status}
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <section className="space-y-6">
      <div>
        ...
        <h2 className="text-2xl font-bold">Roles & permission matrix</h2>
        <p className="mt-2 max-w-3xl text-sm text-slate-600">
          API hỗ trợ list/create/edit/soft delete/gán permissions. Trang này hiển thị danh sách,
          permission matrix và users trong role.
        </p>
      </div>
      <AdminFilters>
        <AdminSearch defaultValue={data.search} placeholder="Tìm role" />
        <span />
      </AdminFilters>
      <AdminDataTable
        columns={columns}
        items={data.items}
        empty={<AdminEmptyState title="Không có role" />}
      />
      <AdminPagination
        page={data.page}
        totalPages={data.totalPages}
        basePath="/admin/roles"
        searchParams={{ search: data.search }}
      />
      <div className="overflow-hidden rounded-2xl border bg-white shadow-sm">
        <div className="border-b p-4 font-semibold">Permission matrix</div>
              ...
              {data.permissions.map((permission) => (
                <tr key={permission.id}>
                  <td className="px-4 py-3">
                    <div className="font-medium">{permission.slug}</div>
                  ...
                  {data.items.map((role) => (
                    <td key={role.id} className="px-4 py-3">
                      {role.permissions.some((item) => item.permissionId === permission.id) ? (
