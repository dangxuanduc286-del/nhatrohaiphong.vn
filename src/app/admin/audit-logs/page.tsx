...
export default async function AdminAuditLogsPage({
  ...
  searchParams?: Promise<{ search?: string; entityType?: string; action?: string; page?: string }>;
}) {
  ...
  const columns: AdminTableColumn<AuditLogRow>[] = [
    ...
    },
    {
      ...
      render: (item) => (
        <div>
          {item.entityType}
          {item.entityId ? <div className="text-xs text-slate-500">{item.entityId}</div> : null}
        </div>
      ),
    },
    {
      key: "user",
      label: "User",
      render: (item) => item.user?.fullName ?? item.user?.email ?? "System",
    },
  ...
  return (
    <section className="space-y-6">
      <div>
        <p className="text-sm font-medium uppercase tracking-wide text-slate-500">Audit Logs</p>
        <h2 className="text-2xl font-bold">Audit logs</h2>
        <p className="mt-2 max-w-3xl text-sm text-slate-600">
          Read-only audit trail: user/action/entity/timestamp/IP, hỗ trợ search/filter/pagination.
        </p>
      </div>
      <AdminFilters>
      ...
      <div className="grid gap-6 xl:grid-cols-4">
        <div className="rounded-2xl border bg-white p-5 shadow-sm">
          <h3 className="font-semibold">Entity phổ biến</h3>
        ...
        <div className="xl:col-span-3">
          <AdminDataTable
            columns={columns}
            items={data.items}
            empty={<AdminEmptyState title="Không có audit log" />}
          />
        </div>
      </div>
      <AdminPagination
        ...
        searchParams={{ search: data.search, entityType: data.entityType, action: data.action }}
      />
    </section>
  );
}
