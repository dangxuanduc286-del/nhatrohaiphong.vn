  ...
  const [items, total] = await Promise.all([
    db.ward.findMany({
      ...
      include: {
        ...
        _count: { select: { properties: true, rooms: true } },
      },
    }),
    db.ward.count({ where }),
  ]);
  ...
  return (
    <section className="space-y-6">
      <div>
        ...
        <h2 className="text-2xl font-bold">Quản lý wards</h2>
      </div>
      <AdminFilters>
        <AdminSearch defaultValue={data.search} placeholder="Tìm ward" />
        <span />
      </AdminFilters>
      <AdminDataTable
        columns={columns}
        items={data.items}
        empty={<AdminEmptyState title="Không có ward" />}
      />
      <AdminPagination
        page={data.page}
