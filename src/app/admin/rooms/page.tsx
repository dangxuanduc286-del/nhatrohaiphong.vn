...
export default async function AdminRoomsPage() {
  await requireAdminPage("room.moderate");
  const [rooms, reports, auditLogs, counts] = await Promise.all([
    ...
    db.report.findMany({
      where: { deletedAt: null, status: { in: ["OPEN", "REVIEWING"] } },
      orderBy: { createdAt: "desc" },
      take: 20,
      include: {
        room: { select: { title: true, roomCode: true } },
        user: { select: { fullName: true, email: true } },
      },
    }),
    db.auditLog.findMany({
      where: { entityType: "Room" },
      orderBy: { createdAt: "desc" },
      take: 8,
    ...
    <section className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm text-slate-600">
          Action dùng server action có permission room.moderate, confirmation trình duyệt, trạng
          thái submit/loading native và audit log. Không xóa dữ liệu phòng.
        </p>
      </div>

      <div className="grid gap-3 sm:grid-cols-4">
        {[
          { label: "Tổng tin", value: counts[0] },
          { label: "Đang hoạt động", value: counts[1] },
          { label: "Đã ẩn/từ chối", value: counts[2] },
          { label: "Bảo trì", value: counts[3] },
        ].map((item) => (
          <div key={item.label} className="rounded-2xl border bg-white p-4 shadow-sm">
            <p className="text-xs text-slate-500">{item.label}</p>
            <strong className="text-2xl">{item.value.toLocaleString("vi-VN")}</strong>
          </div>
        ))}
      </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {rooms.map((room) => (
                  <tr key={room.id} className="align-top">
                    ...
                    <td className="px-4 py-3">
                      {room.district.name} / {room.ward.name}
                    </td>
                    <td className="px-4 py-3">{formatMoney(room.price)}</td>
                    <td className="px-4 py-3">
                      <span className="rounded-full bg-slate-900 px-2 py-1 text-xs text-white">
                        {room.status}
                      </span>
                    </td>
                    <td className="px-4 py-3">{room.reports.length}</td>
                    <td className="px-4 py-3 text-slate-500">{formatDate(room.updatedAt)}</td>
                    <td className="px-4 py-3">
                ...
                reports.map((report) => (
                  <div key={report.id} className="rounded-xl border p-3 text-sm">
                    ...
                    <div className="mt-1 text-xs text-slate-500">
                      {report.room?.title ?? "Không gắn phòng"}
                    </div>
                    <div className="mt-2 inline-flex rounded-full bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-900">
                      {report.status}
                    </div>
                  </div>
                ))
              ) : (
                <p className="text-sm text-slate-500">Không có report mở.</p>
              )}
            </div>
          </div>
          <div className="rounded-2xl border bg-white p-5 shadow-sm">
            <h3 className="font-semibold">Trạng thái action</h3>
            <p className="mt-2 text-sm text-slate-600">
              Khi submit, browser hiển thị pending theo form native; lỗi server hiển thị qua error
              boundary/response, thành công revalidate danh sách và ghi audit log.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {auditLogs.length ? (
            ...
            <p className="text-sm text-slate-500">Chưa có audit log phòng.</p>
          )}
        </div>
      </div>
    </section>
