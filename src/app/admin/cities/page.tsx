...
async function getCities(searchParams: URLSearchParams) {
  ...
  const [items, total] = await Promise.all([
    db.city.findMany({
      ...
      take,
      orderBy: { name: "asc" },
      include: { _count: { select: { districts: true, pointsOfInterest: true } } },
    }),
    db.city.count({ where }),
  ]);
...
export default async function AdminCitiesPage({
  ...
  const columns: AdminTableColumn<CityRow>[] = [
    ...
    },
    {
      ...
      label: "Status",
      render: (item) => (
        <AdminStatusBadge value={item.status} tone={item.status === "ACTIVE" ? "green" : "amber"} />
      ),
    },
  ...
  return (
    <section className="space-y-6">
      <div>
        ...
        <h2 className="text-2xl font-bold">Quản lý cities</h2>
      </div>
      <AdminFilters>
        <AdminSearch defaultValue={data.search} placeholder="Tìm city" />
        <span />
      </AdminFilters>
      <AdminDataTable
        columns={columns}
        items={data.items}
        empty={<AdminEmptyState title="Không có city" />}
      />
      <AdminPagination
        page={data.page}
