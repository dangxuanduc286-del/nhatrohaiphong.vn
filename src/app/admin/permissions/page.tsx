...
export default async function AdminPermissionsPage({
  ...
  await requireAdminPage("permission.manage");
  ...
  const columns: AdminTableColumn<PermissionRow>[] = [
    ...
    },
    {
      key: "group",
      label: "Group",
      render: (item) => <AdminStatusBadge value={groupFromSlug(item.slug)} tone="blue" />,
    },
    {
      key: "roles",
  ...
  return (
    <section className="space-y-6">
      <div>
        ...
        <h2 className="text-2xl font-bold">Quản lý permissions</h2>
        <p className="mt-2 text-sm text-slate-600">
          Danh sách, search, filter theo group và xem role đang được gán.
        </p>
      </div>
      <AdminFilters>
        <AdminSearch defaultValue={data.search} placeholder="Tìm permission" />
        <select
          name="group"
      ...
      <AdminDataTable
        columns={columns}
        items={data.items}
        empty={<AdminEmptyState title="Không có permission" />}
      />
      <AdminPagination
        page={data.page}
        totalPages={data.totalPages}
        basePath="/admin/permissions"
        searchParams={{ search: data.search, group: data.group }}
      />
    </section>
  );
}
