...
export function RoomLocationPicker({
  defaultLatitude,
  defaultLongitude,
  defaultAddress,
  ...
  function useCurrentLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        setStatus(
          "Đã lấy vị trí. Có thể kéo/chỉnh marker bằng cách nhập lại tọa độ sau khi chọn trên bản đồ.",
        );
      },
      () =>
  ...
  function searchAddress() {
    ...
    setStatus(
      "Đã mở Google Maps để tìm địa chỉ. Sao chép tọa độ vị trí phù hợp vào form nếu cần tinh chỉnh.",
    );
  }
    ...
    <section className="rounded-2xl border bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-xl font-bold text-slate-950">
            Gắn vị trí chính xác trên bản đồ
          </h2>
          <p className="mt-2 text-sm text-slate-600">
            Dữ liệu lưu vào trường latitude/longitude hiện có của Room, không thay đổi schema.
          </p>
        </div>
        <Button
          type="button"
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_320px]">
        <div className="overflow-hidden rounded-2xl border bg-slate-100">
          <iframe
            title="Bản đồ chọn vị trí phòng"
            src={mapUrl}
        ...
        <div className="space-y-4">
          ...
          <Button
            ...
            className="w-full rounded-xl border px-4 py-2 text-sm font-semibold hover:border-blue-300 hover:text-blue-700"
          >
            Tìm kiếm địa chỉ
          </Button>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
            <label className="block text-sm font-medium text-slate-700">
              ...
              <Input
                name="latitude"
                value={latitude}
                onChange={(event) => setLatitude(normalizeCoordinate(event.target.value, -90, 90))}
                className="mt-1 w-full rounded-xl border px-3 py-2 text-sm"
                inputMode="decimal"
              />
            </label>
            <label className="block text-sm font-medium text-slate-700">
              ...
              <Input
                ...
                value={longitude}
                onChange={(event) =>
                  setLongitude(normalizeCoordinate(event.target.value, -180, 180))
                }
                className="mt-1 w-full rounded-xl border px-3 py-2 text-sm"
                inputMode="decimal"
              />
