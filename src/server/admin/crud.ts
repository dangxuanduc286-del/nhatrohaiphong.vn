...
export const adminModelConfig = {
  ...
  permission: { permission: "permission.manage", entity: "Permission" },
    ...
    await tx.auditLog.create({
      data: {
        ...
        oldValues: {
          name: existing.name,
          slug: existing.slug,
          permissionIds: existing.permissions.map((item) => item.permissionId),
        },
        newValues: body,
      },
    });
...
export async function assignPermissionRoles(
  permissionId: string,
  input: unknown,
  adminUserId: string,
) {
  ...
  await db.$transaction(async (tx) => {
    ...
    if (body.roleIds.length)
      await tx.rolePermission.createMany({
        data: body.roleIds.map((roleId) => ({ roleId, permissionId })),
        skipDuplicates: true,
      });
    await tx.auditLog.create({
      data: {
        userId: adminUserId,
        action: "ADMIN_PERMISSION_ASSIGN_ROLES",
        entityType: "Permission",
        entityId: permissionId,
        oldValues: { roleIds: existing.roles.map((item) => item.roleId) },
        newValues: body,
      },
    });
  });
...
export async function assignUserRoles(input: unknown, adminUserId: string) {
  ...
  const roles = body.roleIds.length
    ? await db.role.findMany({
        where: { id: { in: body.roleIds }, deletedAt: null },
        select: { id: true },
      })
    : [];
  ...
  await db.$transaction(async (tx) => {
    ...
    await tx.userRole.create({ data: { userId: body.id, roleId: body.roleIds[0] } });
    await tx.auditLog.create({
      data: {
        userId: adminUserId,
        action: "ROLE_ASSIGN",
        entityType: "User",
        entityId: body.id,
        oldValues: { email: existing.email, roleIds: existing.roles.map((item) => item.roleId) },
        newValues: { roleIds: body.roleIds },
      },
    });
  });
}
...
export async function createLandingPage(input: unknown, adminUserId: string) {
  ...
  return db.$transaction(async (tx) => {
    ...
    await tx.auditLog.create({
      data: {
        userId: adminUserId,
        action: "ADMIN_LANDING_PAGE_CREATE",
...
export async function updateLandingPage(id: string, input: unknown, adminUserId: string) {
  ...
  return db.$transaction(async (tx) => {
    ...
    const page = await tx.landingPage.update({
      where: { id },
      data: {
        path: body.path,
        title: body.title,
    ...
    await tx.auditLog.create({
      data: {
        userId: adminUserId,
        action: "ADMIN_LANDING_PAGE_UPDATE",
        entityType: "LandingPage",
        entityId: id,
        oldValues: { path: existing.path, title: existing.title },
        newValues: body,
      },
    });
    return page;
...
export async function softDeleteLandingPage(id: string, adminUserId: string) {
  ...
  await db.auditLog.create({
    data: {
      userId: adminUserId,
      action: "ADMIN_LANDING_PAGE_SOFT_DELETE",
      entityType: "LandingPage",
      entityId: id,
      oldValues: { path: page.path },
    },
  });
  return updated;
}

export async function upsertPoiSeo(
  tx: Prisma.TransactionClient,
  entityId: string,
  body: { name: string; description?: string | null; slug: string },
) {
  const existing = await tx.sEOSetting.findFirst({
    where: { entityType: "PointOfInterest", entityId, deletedAt: null },
  });
  const data = {
    ...
    canonicalUrl: `/dia-diem/${body.slug}`,
    keywords: [body.name],
  };
...
export async function softDeleteDistrict(id: string, adminUserId: string) {
  ...
  await db.auditLog.create({
    data: {
      userId: adminUserId,
      action: "ADMIN_DISTRICT_SOFT_DELETE",
      entityType: "District",
      entityId: id,
      oldValues: { name: existing.name, slug: existing.slug },
    },
